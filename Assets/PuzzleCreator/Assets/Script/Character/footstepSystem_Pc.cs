//Description : footstepSystem_Pc. Allow to manage player foostep sound depending the surface
using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class footstepSystem_Pc : MonoBehaviour {
		...
		if (CheckTag (_tag)) {
			for (var i = 0; i < listCompareMangnitude.Count; i++) {
				if (bodyVelocity.magnitude > listCompareMangnitude[i].listCharacterMangnitude) {
					if (!_audio.isPlaying) {
						if (_Timer == listCompareMangnitude[i].listTimeBetweenTwoStep) {
						
							playSound (ChooseSound ());
							_Timer = 0;
						} else {
							_Timer = Mathf.MoveTowards (_Timer, listCompareMangnitude[i].listTimeBetweenTwoStep, Time.deltaTime);
						}
					}
				}
			}
