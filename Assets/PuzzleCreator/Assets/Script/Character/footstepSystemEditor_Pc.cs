//Description : footstepSystemEditor_Pc. Work in association with footstepSystem_Pc. Allow to manage player foostep sound depending the surface
#if (UNITY_EDITOR)
using UnityEngine;
...
[CustomEditor(typeof(footstepSystem_Pc))]
public class footstepSystemEditor_Pc : Editor {
	...
	public override void OnInspectorGUI()
	{
		...
		GUILayout.Label("");
        footstepSystem_Pc myScript = (footstepSystem_Pc)target; 
...
//--> ADd new sound on a specific surface
	private void AddSound(footstepSystem_Pc myScript,int i,int j){
...
//--> Remove a specific sound on a specific surface
	private void RemoveSound(footstepSystem_Pc myScript,int i,int j){
		Undo.RegisterFullObjectHierarchyUndo (myScript, myScript.name);
